	...
	if len(s) > 3 && s[1] == char && s[2] == char {
		...
		if minIndent > 0 {
			for i, line := range lines {
				lines[i] = escapeContent(line[minIndent:])
			}
		}
...
func escapeContent(raw []byte) []byte {
	buffer := bytes.NewBuffer([]byte(""))
	start := 0
	for i := 0; i < len(raw); {
		if b := raw[i]; b < utf8.RuneSelf {
			if safeSet[b] {
				i++
				continue
			}
			if start < i {
				buffer.Write(raw[start:i])
			}
			buffer.WriteByte('\\')
			switch b {
			case '\\', '"':
...
// Below character set is borrowed from encoding/json
// ------------------------------------------------------------------------
